...
document.getElementById("save").onclick=function(){
    ...
    for(let i=1;i<tb.rows.length;i++){
        ...
        for(let j=tb.rows[i].cells.length-1;j>=tb.rows[i].cells.length-12;j--){
            obj.sale.unshift(tb.rows[i].cells[j].childNodes[0].value);
        }
        data.push(obj);
    }
...
function drawTable(data) {
    ...
    for(let i=0;i<data.length;i++){
        ...
        for(let j=0;j<data[i].sale.length;j++)
            tableHTML+="<td><input "+"value="+data[i].sale[j]+"></input></td>";
        tableHTML+="</tr>";
    }
    tableHTML+="</table>";
    ...
    if(event.target.nodeName=="TD"){
        ...
        if(index!=0){
            for(let i=tb.rows[index].cells.length-12;i<tb.rows[index].cells.length;i++){
                data.push(tb.rows[index].cells[i].childNodes[0].value);
            }
        }
        drawBar(data);
    }
...
function drawLine(){
    ...
    for(let i=1;i<tb.rows.length;i++){
        ...
        for(let j=tb.rows[i].cells.length-12;j<tb.rows[i].cells.length;j++){
            lineData.push(parseInt(tb.rows[i].cells[j].childNodes[0].value));
        }
        data.push(lineData);
    }
